...
namespace ESI.NET.Models.FactionWarfare
{
    ...
    {
        [JsonProperty("kills")] public Summary<T> Kills { get; set; }

        [JsonProperty("victory_points")] public Summary<T> VictoryPoints { get; set; }
    }

    public class Summary<T>
    {
        [JsonProperty("yesterday")] public List<T> Yesterday { get; set; } = new List<T>();

        [JsonProperty("last_week")] public List<T> LastWeek { get; set; } = new List<T>();

        [JsonProperty("active_total")] public List<T> ActiveTotal { get; set; } = new List<T>();
    }
